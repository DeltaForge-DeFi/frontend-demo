...
import { RiArrowDownLine, RiArrowDownSFill, RiArrowRightLine, RiArrowRightSFill, RiArrowUpLine, RiArrowUpSFill } from '@remixicon/react';
...
const badgeDeltaVariants = cva('inline-flex items-center text-tremor-label font-semibold', {
  ...
  compoundVariants: [
    ...
    // Solid variants
    {
      deltaType: 'increase',
      variant: 'solid',
      className: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-400/20 dark:text-emerald-500',
    },
    {
      deltaType: 'decrease',
      variant: 'solid',
    ...
    },
    {
      deltaType: 'neutral',
      variant: 'solid',
      className: 'bg-gray-200/50 text-gray-700 dark:bg-gray-500/30 dark:text-gray-300',
    },
    // Solid outline variants
    {
      deltaType: 'increase',
      variant: 'solidOutline',
      className: 'bg-emerald-100 text-emerald-800 ring-emerald-600/10 dark:bg-emerald-400/20 dark:text-emerald-500 dark:ring-emerald-400/20',
    },
    {
      deltaType: 'decrease',
      variant: 'solidOutline',
      className: 'bg-red-100 text-red-800 ring-red-600/10 dark:bg-red-400/20 dark:text-red-500 dark:ring-red-400/20',
    },
    {
      deltaType: 'neutral',
      variant: 'solidOutline',
      className: 'bg-gray-100 text-gray-700 ring-gray-600/10 dark:bg-gray-500/30 dark:text-gray-300 dark:ring-gray-400/20',
    },
  ],
});

interface BadgeDeltaProps extends React.HTMLAttributes<HTMLSpanElement>, VariantProps<typeof badgeDeltaVariants> {
  value: string | number;
}
      ...
      <span className={cn(badgeDeltaVariants({ variant, className }))} {...props}>
        <span
          className={cn(
            ...
            deltaType === 'increase' && 'text-emerald-700 dark:text-emerald-500',
            deltaType === 'decrease' && 'text-red-700 dark:text-red-500',
            deltaType === 'neutral' && 'text-tremor-content-emphasis dark:text-dark-tremor-content-emphasis',
          )}
        >
          {value}
        </span>
        <span
          className={cn(
            ...
            deltaType === 'neutral' && 'bg-tremor-background-subtle dark:bg-dark-tremor-background-subtle',
          )}
        >
          {/* @ts-ingore */}
