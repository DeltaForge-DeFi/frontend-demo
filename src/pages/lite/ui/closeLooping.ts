...
export const closeLooping = async () => {
    ...
    for (let index = 0; index < 1; index++) {
      ...
      dsProxyExecute(executeContractWithdraw, executeDataWithdraw).catch(
        (error) => {
...
///////////////////
      ...
      dsProxyExecute(executeContractPayback, executeDataPayback).catch(
        (error) => {
  ...
  const calculateSafeWithdrawAmount = async (params: {
    ...
    const requiredCollateralUsd =
      (params.totalDebt *
        BigInt(Math.floor(params.minHealthFactor * 100)) *
        BigInt(100)) /
      params.liquidationThreshold;
    const maxWithdrawUsd =
      params.totalCollateral > requiredCollateralUsd
        ? params.totalCollateral - requiredCollateralUsd
        : BigInt(0);
  
    // 2. Определяем сколько нужно свапнуть в USD
    const swapAmountUsd =
      maxWithdrawUsd < params.totalDebt ? maxWithdrawUsd : params.totalDebt;
    ...
    console.log(
      "Текущий коллатерал (USD):",
      formatUnits(params.totalCollateral, 8)
    );
  ...
  const AAVE_WITHDRAW_ADDRESS =
    "0x570BfB7A185EFa93d54b06348a9eB69F6bd94ec3" as Address;
